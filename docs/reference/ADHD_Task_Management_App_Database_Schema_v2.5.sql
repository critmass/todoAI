-- ADHD Task Management App - SQLite Database Schema v2.5
...
--
-- VERSION NOTE: this snapshot's own filename/header version (v2.5) tracks schema_metadata's
-- 'version' key (2.5.0, set by migration 004 below) -- NOT the specification version. The
-- companion spec is still at v2.3 (docs/reference/ADHD_Task_Management_App_Specification_v2.3.md)
-- as of this snapshot; the spec and the schema version different things (product-facing rulings
-- vs. applied DDL state) and legitimately drift out of lockstep. Do not read "v2.5 schema, v2.3
-- spec" as one of them being wrong -- reconcile by checking migration history and the fold-in
-- task briefs (task 27 folded 001+002; this snapshot adds 003+004), not by assuming the numbers
-- should match.
--
-- Changes from v2.3 (schema_metadata version 2.3.0 -> 2.5.0, applied by migrations 003 and 004):
--
-- Migration 003 (v2.3 -> v2.4, src/db/migrations/003_multisession_work.sql -- task 28's
-- multi-session-work / hyperfocus-extension design, task 33):
--   1. tasks.duration_type       - 'estimate' | 'floor' (open-ended work; estimated_duration
--                                  holds the floor value when 'floor'). Spec SS3.1.
--   2. tasks.work_state          - 'none' | 'in_progress'. Orthogonal to status; a parked task
--                                  stays status='active' so every pool query works unchanged.
--   3. tasks.accumulated_minutes - minutes worked toward the current completion; folds to ONE
--                                  actual_duration_history entry at completion (spec SS2).
--   4. tasks.last_worked_at      - re-anchors the neglect clock (working a task is attention);
--                                  a third MAX() input alongside created_at/last_completed_at in
--                                  listActiveByNeglect (spec SS5, task 33's anchor merge).
--   5. sessions.tasks_progressed - parked tasks count here, not in tasks_completed/tasks_skipped.
--   6. interactions               - interaction_type gains 'task_progress', completion_status
--                                  gains 'progress' (the park episode's interaction row).
--
-- Migration 004 (v2.4 -> v2.5, src/db/migrations/004_algorithm_weights_reconciliation.sql --
-- task 34, this snapshot):
--   1. algorithm_weights.factor_name CHECK drops 'context_fit' -- R3 (task 10) removed context
--      fit from the weighted sum entirely; context/tools are now a hard pre-filter at the
--      selection boundary (src/scoring/filter.ts), not one of the summed factors. The seed row
--      for the retired factor is deleted (see the migration's ASYMMETRY note: unconditionally,
--      regardless of data_points_count -- the factor has nowhere for learned data to go).
--   2. algorithm_weights seed data updated to 31/23/23/23 (importance/urgency/energy_match/
--      historical_success), matching src/scoring/factors.ts's FACTOR_WEIGHTS -- but ONLY for
--      rows where data_points_count = 0 (i.e. nothing has been learned yet). A row the numeric-
--      learning loop (task 17, not yet built) has already updated is left untouched.
--   3. active_tasks_with_neglect view DROPPED. It computed the retired weeks^2 neglect curve via
--      POWER(), a function unavailable on this app's on-device SQLite build, so it could never
--      actually execute; it also predates R1 (linear curve), R8 (accrual gate), and task 28's
--      three-way anchor merge, none of which it reflected. Kept as "stale reference" through v2.3
--      and v2.4, but a view that cannot run and states a superseded rule is a trap for the next
--      reader, not a harmless artifact -- see the migration file's DECISION note for the full
--      reasoning and the counter-argument it weighs. The real computation has always been (and
--      remains) src/db/repositories/tasks.ts's listActiveByNeglect().
--
-- Carried forward from v2.3: learning_state, skills.is_active DEFAULT FALSE, skill_evidence.source,
-- coaching_queue's five-value trigger_type CHECK (migration 002); tasks.duration_source,
-- task_recurrence.recurrence_type (five types) + target_count, corrected schema_metadata strings,
-- parent_task_id, summary_schema_version, skill tables, neglect excluded from summed
-- algorithm_weights, importance 1-1000 / energy 1-5 (v2.2 and earlier).
...
-- estimated_duration: NOT NULL; coach guesses when the user doesn't supply one. Holds the FLOOR
--   value when duration_type='floor' (open-ended work; spec 3.1).
...
-- duration_type / work_state / accumulated_minutes / last_worked_at: task 28's multi-session-work
--   columns (migration 003, task 33). See the table CHECKs below and the header changelog.
...
CREATE TABLE tasks (
    ...
    duration_type TEXT NOT NULL DEFAULT 'estimate' CHECK (duration_type IN ('estimate', 'floor')),
    work_state TEXT NOT NULL DEFAULT 'none' CHECK (work_state IN ('none', 'in_progress')),
    accumulated_minutes INTEGER NOT NULL DEFAULT 0 CHECK (accumulated_minutes >= 0),
...
-- Interactions
-- =====================================================================
-- interaction_type gains 'task_progress' and completion_status gains 'progress' in migration 003
-- (task 28's park episode; spec 8.2/8.7).
...
-- Sessions
-- =====================================================================
-- tasks_progressed (migration 003): parked tasks count here, not in tasks_completed/tasks_skipped.
...
CREATE TABLE sessions (
    ...
    tasks_progressed INTEGER NOT NULL DEFAULT 0
);
...
-- RECONCILED in migration 004 (task 34): factor_name CHECK no longer accepts 'context_fit' (R3
-- moved context/tools to a hard pre-filter, src/scoring/filter.ts) and the seed data matches
-- src/scoring/factors.ts's actual FACTOR_WEIGHTS (31/23/23/23) -- but only where nothing has been
-- learned yet (data_points_count = 0). See the migration file for the full reseed-guard reasoning.
...
-- is_active DEFAULT FALSE (was TRUE through v2.2) -- born-inactive defense-in-depth (migration
-- 002, task 18 design SS2.1). Existing rows keep their actual stored value; only the default changes.
CREATE TABLE skills (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
-- 31/23/23/23 (importance/urgency/energy_match/historical_success) -- migration 004 reconciled
-- this with src/scoring/factors.ts's FACTOR_WEIGHTS (R3). context_fit no longer exists here.
INSERT INTO algorithm_weights (factor_name, weight_percentage) VALUES
    ('importance', 31.0),
    ('urgency', 23.0),
    ('energy_match', 23.0),
    ('historical_success', 23.0);
...
INSERT INTO schema_metadata (key, value) VALUES
    ...
    ('last_migration', 'v2_5_algorithm_weights_reconciliation');
...
-- Views
-- =====================================================================
-- active_tasks_with_neglect, present through v2.4, is DROPPED as of migration 004 -- see the
-- header changelog and the migration file's DECISION note. The real neglect computation lives
-- (and always lived, for real queries) in src/db/repositories/tasks.ts's listActiveByNeglect().
