...
## Appendix — the coordinator's spawn prompt + the StatFs amendment (added for completeness)

*Added by the coordinator 2026-08-19, recording verbatim the inline prompt sent when spawning the Phase 2 subagent, and the follow-up SendMessage amendment that appended the StatFs free-space query. Supplements the pre-existing brief `docs/briefs/event_capture_task_41.md`, the design `docs/design/capture_format_task41.md`, and the rulings amendment `docs/design/capture_format_task41_amendment_rulings.md`.*

### A1 — the Phase 2 spawn prompt

> You are executing **task 41, Phase 2** on the todoAI project (repo root: `C:\Users\physi\Documents\projects\todoAI`, branch `main`). You are the builder, not the coordinator. Jason is the sole developer and decision-maker. You do not have the device — the device run is his. This is the critical path: task 41 gates task 31 (the corpus), which gates 38 and 40. The app currently discards its most valuable data by design; the collection window closes permanently at open beta.
>
> **Read first, in order:** (1) `docs/design/capture_format_task41.md` — the format design, the on-disk contract; (2) the amendment `capture_format_task41_amendment_rulings.md` — Jason's rulings; where it and the parent design disagree, THIS ONE WINS; Phase 1 is complete and fully ruled; (3) `docs/briefs/event_capture_task_41.md` — the original work order; (4) `docs/briefs/orientation_for_opus.md` §3, §4, §5.
>
> **The rulings you build to (amendment §1):** storage = a new app-owned TurboModule with a synchronous append (not `react-native-fs` — a Promise-based `appendFile` loses exactly the events before a native crash); streams = split, twelve streams, one egress class each; crisis near-miss = log every turn the gate ran on, hit or clear (option (c), supersedes parent §5.9); actor attribution = user/coach/system/planner (supersedes parent §5.5); thermal seam = build in-app thermal/battery sampling HERE (overrules parent §4.3); retention = keep everything, single black-swan ceiling warning, no second trigger; fsync = per event for alpha, with a pinned future revert to boundary-only at beta; stale worktree = leave it, flag in your report; global v = RATIFIED by Jason, global across streams, not per-stream (build to parent §9's six rules).
>
> **Four details easy to get subtly wrong:** `planner` is a sentinel whose expected count is ZERO — nothing may be attributed to it by default or by a fallback branch; every enumerated writer is attributed explicitly at the wiring point in `appServices.ts`; a `planner` row must always be a fact about the code, never a shrug. The crisis stream must not touch the detector — capture reads `checkCrisis`'s verdict and writes nothing back; `patternIndex` is deliberately dropped; there are two gates (`runCoachingResolution` runs its own `checkCrisis` at disposition time). Thermal: sampling only, NO policy — `PowerManager.getCurrentThermalStatus()` + a `BatteryManager` read, surfaced into the runtime stream and into `TernaryBonsaiProvider`'s `thermalStatusSampler`; no tier degradation/deferral/gating. The ceiling warning is a black-swan net, not a workflow prompt — rare enough to be alarming, lives inside `src/capture/`, never interrupts mid-episode, is a warning not a block; its non-firing is not evidence of anything.
>
> **Build order (amendment §10):** (1) the force-kill acceptance test FIRST (parent §14.1) — before the implementation, so a buffering regression fails immediately; (2) the TurboModule, `record()`, and the writer; (3) the call sites per brief §6; (4) the widened `LlmOutputValidationError` — an optional third parameter so every existing construction site compiles unchanged; (5) `scripts/pull-capture.js`.
>
> **Constraints:** Removability is a settled decision (orientation §5) — streams separately scoped, each under a directory it owns, one module with one entry point, on-disk layout a written contract, capture logic never diffusing into the code it instruments; task 42 must be able to delete one stream and prove the others survive. Capture failure never breaks the app but never goes silent (the `dropped: {count, lastReason}` mechanism). Bind with `TurboModuleRegistry.get`, NOT `getEnforcing` — under Jest or on an APK built before the module existed, capture becomes a counted no-op, never a launch crash. App-private external storage only. Local-only is untouched — capture is recording, not transmission. The op-sqlite stub in `jest.setup.js` will need `ANDROID_EXTERNAL_FILES_PATH` (the real package destructures its path constants at module import time; it fails at import, which reads like unrelated breakage). The `.cxx` codegen build trap (task 24 §9.6): `codegenConfig` already points at `src/specs/` so a second spec should not re-trigger it, but the runbook says `rm -rf android/app/.cxx` before the first build anyway — say so in your report; Jason runs the build. Do NOT create migration 007 — task 44 owns it; the episode stream's `origin` field is designed to it; if you think you need a schema change, stop and report.
>
> **Verification:** `npx jest`, `npx tsc --noEmit`, `npx eslint .`. Jest count trap (~double, stale worktree; Jason ruled it stays). Current real baseline: 872 tests / 75 suites green (raw 1666/143). Halve any count; check real tree vs duplicate on a failure.
>
> **File scope:** yours is `src/capture/` (new), `src/llm/errors.ts`, `src/llm/provider/ladder.ts`, `src/app/`, `src/execution/`, `src/specs/`, the Android Kotlin sources, `scripts/`, `jest.setup.js`. Not yours (recently landed): `src/services/backup/` and `src/db/testUtils/` (task 14 Phase A), `src/llm/grammar/primitives.ts` (task 48), the four `.gbnf` files (task 37) — if you believe any is wrong, report it, don't edit it. `src/app/` is contended: task 44 also edits `sessionController.startSession`, and task 14's report §13 describes a 3-call-site wiring recipe for its session gate that is deliberately not wired yet and is not yours to wire; keep your `src/app/` footprint minimal and enumerate every file you touch there so 44 can rebase cleanly.
>
> **Deliverable:** `docs/eval/task41_findings_report.md`. It already owes three specific items (amendment §10): the thermal deviation (orientation §8 pinned the sampler to task 19 and Jason reassigned it here — attribute it explicitly as his instruction, not your judgment); the halved test count; and app-open time with capture on and off (per-event fsync bursts on the recurrence sweep at app open are the real risk; that number decides whether the beta revert gets pulled forward). A section titled exactly "Deviations from human decisions" — separate from "decisions this task had to make" and "deferred deliberately"; empty is a valid answer and must be written out explicitly. Code comments must cite the document that authorises them. Separate believed from confirmed — the force-kill test exercises a Node writer, not the Kotlin one; say plainly what that does and does not prove. Commit at natural breakpoints; do not push. If the design is wrong about something, say so plainly with the mechanism.

### A2 — the StatFs amendment (follow-up SendMessage)

> Small amendment to task 41 Phase 2, ruled by Jason 2026-08-18. Your Phase 2 work is accepted — this adds one method before the device build. Task 14 (backup/restore) needs free disk space and nothing in this tree reports it; its ladder ships attempt-and-catch (`VACUUM INTO` then catch `SQLITE_FULL`), which gives no advance warning. Task 14's report recommended bundling a ~30-line `StatFs` TurboModule with your `NativeCaptureLog` rebuild rather than spending a separate device build; Jason ruled yes; your module is written but the device build hasn't happened, so the bundling window is open now.
>
> **Scope — deliberately narrow:** add a free-space query to the existing `NativeCaptureLog` module — `availableBytesFor(path: string): number` (or a path-scoped variant if you judge that better) over `StatFs` on the capture/app-private volume, plus the JS accessor. **Expose it only. Do NOT consume it.** Do not touch `src/services/backup/` (task 14's, call sites deliberately unwired); do not wire it into the backup ladder or any session gate; do not add a policy, threshold, or warning UI. Your existing ceiling warning is unchanged and is a different mechanism.
>
> **Constraints that still apply:** same codegen-mapping caution you already exercised (you used `listDaysCsv(): string` rather than an array return because a sync array return has no working example in this repo; a Double/number return has precedent in `EpisodeAlarmModule`); if the value can exceed what a JS number represents safely, say so and pick the representation deliberately. `TurboModuleRegistry.get`, not `getEnforcing` — degrade to a counted no-op under Jest and on an APK built before the method existed. Degradation must be legible to the caller — task 14 needs to distinguish "free space is 40 MB" from "I cannot tell you"; collapsing them into `0` would make a missing module look like a full disk, the worst possible confusion for a blocking decision; choose a representation that keeps them apart and state it.
>
> **Deliverable:** update `docs/eval/task41_findings_report.md` with a short section covering the method, its degradation behaviour, and the representation choice; if this changes anything in your "Deviations from human decisions" section, update it, otherwise leave that section as written. Note that task 14 is the consumer and the wiring is still unbuilt, so a future session doesn't assume the free-space check is live. Task 44 is running right now and owns `src/app/` and migration 007 — stay out of both; your files here are `src/specs/NativeCaptureLog.ts`, the Kotlin sources, and whatever thin JS accessor you need in `src/capture/`. Re-run the checks (baseline 910 real tests / 80 suites; halve for the stale worktree). Commit, do not push. One correction to your report while you are in there: it says orientation §8's thermal pin still names task 19 and needs a coordinator edit — that edit was already made (§8 now reads "REASSIGNED to task 41, ruled by Jason 2026-08-17"); your deviation 2.1 entry is otherwise exactly right and should stay.
