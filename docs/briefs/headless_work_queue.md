...
| 0 | **Housekeeping** *(unnumbered)* — worktree cleanup + flake hunt | Opus | `housekeeping_worktrees_and_flake.md` ✅ | Remove the 3 merged agent worktrees **and** the long-standing stale one (discharging Jason's 2026-08-17 "separate cleanup task later"), so raw `npx jest` finally reports the true number; then an honest attempt at the one unreproduced failure. ⚠ **Runs BEFORE wave 2** — worktree deletion cannot run while worktree-isolated agents hold them. | 🟡 **RUNNING** |
