...
describe('MIGRATION_006_SQL', () => {
  it('is byte-identical to 006_recurrence_period.sql', () => {
...
describe('MIGRATION_008_SQL', () => {
  it('is byte-identical to 008_transitive_cycle_guard.sql', () => {
    const sqlPath = path.join(__dirname, '..', '008_transitive_cycle_guard.sql');
    const fileContents = fs.readFileSync(sqlPath, 'utf-8');
    expect(lf(MIGRATION_008_SQL)).toBe(lf(fileContents));
  });
});
