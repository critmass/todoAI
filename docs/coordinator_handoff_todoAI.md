...
*Refreshed 2026-08-07 for the Claude Code handoff (that note is below). Re-refreshed 2026-08-19 (device session; the execution boundary ruled). **Re-refreshed 2026-08-24** — a long headless session run almost entirely through briefed subagents: **task 14's §13 wiring landed** (the backup ladder is finally invoked by the app), **task 50 closed** (the `energy` definition, which unblocked 31), **`CLAUDE.md` added** (a repo-wide test-first policy that did not exist), **task 53's test-suite audit** found 12 tests that would pass against broken code and spawned 54–59 — all now done, **task 17 Phase A** closed a live one-sided scoring bug, **migration 008** widened the circular-dependency guard, **all git worktrees were removed** (raw `npx jest` is finally the true number) and the repo was packed 15 MB → 3.9 MB, and **task 46 was expanded to four recurrence modes** with its engine landed. Suite **794 → 1121 tests**. The 2026-08-19 first-actions box is fully discharged and replaced.*

> ### ⚠ First actions, in order (as of 2026-08-24)
>
> 0. **The execution boundary still governs (§1).** You brief subagents and verify; you do not author code, run builds, or drive the device. You MAY run `jest`/`eslint`/`tsc` to check a claim — and this session shows why that matters: a subagent-reported green hid a real failure once, and a "the content is merged" claim of the coordinator's own was worth re-proving file-by-file before anything was deleted.
> 1. **Read `CLAUDE.md` — it is new and it is repo-wide.** Test-first for every behaviour change: write the failing test, watch it fail for the right reason, then implement; carve-outs must be *stated* in the report, never silent. **Every brief you write says so.** It exists because there was no such policy and the only test-first instances were per-task accidents.
> 2. **The queue is `docs/briefs/headless_work_queue.md`.** It is the running order and it records how the work runs: one written brief per subagent, a minimal prompt pointing at it, a returned report, coordinator verification, then merge. **Waves run in parallel via `isolation: "worktree"`** — file-disjointness at merge time is the constraint, not the shared tree. Keep waves small: the real limit is how many reports you can verify *properly*.
> 3. **Next up: task 46 Phase 2** — the recurrence editor. 🔴 Until it lands, the four new recurrence modes are **unreachable by a user**, which is precisely the state task 14's backup ladder sat in for weeks and which this session spent real effort undoing. Jason has ruled the shape: a **top-line dropdown** carrying every option, with the row beneath re-shaping to the selection; a **31-cell checkbox grid** for dates; and a **6×7 grid** (Sunday–Saturday × 1st/2nd/3rd/4th/5th/Last) for weekdays-of-month. ⚠ **The dropdown must be JS-only** (`Pressable` + core `Modal`) — a native picker means a rebuild and this project has a documented `.cxx` codegen trap.
> 4. **Decisions owed by Jason** (surface, don't decide) — the board's "Open rulings owed" is authoritative: task 46 Phase 1's deviation (the new modes deliberately do not roll `reset_date`/`last_period_shortfall`, because nothing increments the progress they'd be measured against) plus the 30th/31st clamp collision and anchor-aligned interval weeks; task 17 Phase A's attempt definition (does a self-completion count toward `success_rate`?); task 14's two ladder-policy properties; the `use_breath_pause` tag; the four task-41 provisional builder calls; whether to run task 14's destructive Phase B now that it is wired; and the extraction guide still being over its token budget.
> 5. **The strategic thread is `31 → 38 → 40`, and 31 is FULLY UNBLOCKED** — 41 (capture) and 50 (the `energy` definition) are both done, so nothing gates it. It is the longest-lead item on the board and the only thing standing between here and the model-migration decision. Brief + copy-paste prompt: `docs/briefs/real_task_corpus_task_31.md` and `docs/briefs/task_31_session_init_prompt.md`, both refreshed for the post-50 world. **It needs Jason's time, not a subagent's.**
> 6. **Device work is unscheduled and accumulating.** Task 14 Phase B is now *reachable* (wired this session) but unrun; also pending: StatFs's two-volume check (still zero callers — the gate is attempt-and-catch), the app-open `fsync` number, thermal-under-load, blocked-button states, and the **"Wrap this up" defect lead** (not a dead button — a 15–83 s model call whose only feedback is a caption). Batch them into task 32.
...
> 🔴 **The execution boundary — RULED by Jason, 2026-08-19 (refined same day).** The coordinator **never changes code, never runs builds, and never drives the device directly.** Code authoring, builds, and device execution are subagent tasks. This is not new in spirit — "you do not write app code" always meant it — but it now explicitly covers **builds and device driving** too, because in Claude Code the coordinator *can* execute and this session drifted into driving the gradle build, `adb input`, and editing `scripts/gen-task-table.js` directly.
> - **Never:** edit `src/`, `android/`, native, or `scripts/*` (incl. `gen-task-table.js`); run `gradlew`/any build; build/install/drive the device (`adb install`/`input`/`am force-stop`/instrumented checks).
> - **MAY (refinement — Jason corrected the first draft, which had over-banned this):** run the **`jest` suite, `eslint`, and `tsc --noEmit`** directly, for **independent verification** of a subagent's claim — running a read-only check to confirm a report is verification, not authoring (it caught the 794-vs-1588 stale-worktree trap this session). Read-only `adb` pulls to gather an artifact for review are fine in the same spirit; `adb` that builds/installs/drives is not.
> - **Do:** maintain the canonical record (markdown — board, orientation, briefs, findings reports, this handoff); **run `node scripts/gen-task-table.js`** to render the board; run `jest`/`eslint`/`tsc` to verify; read code and reports; write briefs; review subagents' reports + diffs; interface between Jason and the subagents.
> - 🔴 **A brief for every subagent; a report from every subagent.** Every spin-up gets a **written brief** (a durable record of what was asked — a `docs/briefs/` file, not just an inline prompt), and every subagent **returns a report** (a durable record of what it did). The two are the audit trail. When you augment an existing task brief with coordination context (file-scope, the 794/1588 trap, the deviations rule), fold it into a brief file, don't leave it only inline. *(This session's device subagents used inline prompts with no brief — that is the gap this rule closes.)* **And every brief states the test-first policy (`CLAUDE.md`, added 2026-08-22):** for any behavior change the subagent writes the failing test first and proves it fails before the code, and names the guarding test in its report; carve-outs (doc/prompt/config-only edits, refactors under existing tests, `src/dev/` spikes) must be stated, not silent.
> - **Verification is reading first**, then — if a check is needed — you may run the test suite / lint / typecheck yourself, or brief a subagent for a build/device measurement and review the result. A device subagent builds/drives/pulls and reports; you review.
> - **Why this wasn't an issue before:** the coordinator ran in chat / Claude Cowork with Jason spinning up the build agents by hand — the boundary was enforced by the environment. In Claude Code it must be enforced by discipline. It keeps the reviewer from also being the *author*, while independent test runs keep *verification* honest.
> - **Only Jason overrides the authoring/build/device boundary, per instance** (as he did with the gradle build this session). **This supersedes the older "you can run `adb` now" framing below** (§2 tooling table, the closing line).
...
| Could not run the test suite — Windows-built native modules, slow repo mount | **`npx jest` runs on this tree — and the coordinator MAY run it (plus `eslint`/`tsc`) to verify a subagent's claim** (§1 refinement, 2026-08-19). Builds and device driving still go to subagents. |
...
*You can run the tests to verify (jest/eslint/tsc) — that independence is worth keeping.* ⚠ **But per the §1 execution boundary (2026-08-19), the coordinator does NOT author code, run builds, or drive the device — those go to subagents, each with a written brief and a returned report.** *Start by bringing Jason the tradeoff with a recommendation, surface every deviation from what he decided, and check for a newer report before acting on any finding.*
