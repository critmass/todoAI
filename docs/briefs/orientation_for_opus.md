...
**Last refreshed:** 2026-07-27, after task 13 landed (timer engine confirmed on-device) and the task 23 design prototype was reviewed. If the master table shows tasks this doc's §2 doesn't, trust the table and fix §2.
...
| 8, 14, 15, 17 | tiering / data-resilience / numeric learning | see the master task table for allocation + dependencies. **Task 17 handoff (from 13):** `completion_count` and `success_rate` have **no writer anywhere** — `historicalSuccessFactor` scores every task off a permanent n=0; 17 is the natural owner of the writer (13 report §7) |
...
| 23 | UI/UX design (interaction + visual system) | 🟡 **design substantially done** — interactive prototype at `docs/design/Main Screen.dc.html` + `Coaching Screen.dc.html`; review at `docs/eval/task23_review.md`. All six reconciliations with the rulings verified in-code. **Three small follow-ups open** (guardrail-B nudge, repeated-+5 note, skip threshold 2→3) — a Claude Design prompt for them is in the review §5. §9 |
| 24 | Product UI implementation (real screens) | ⬜ **not started** — brief `docs/briefs/product_ui_task_24.md`. **Functional pass gates personal ship** (only dev screens exist today). Consumes the task 23 prototype + task 13's `src/execution/` API. Carries `P`. §9 |

**Merged-branch state:** 11, 34, and 13 landed on `opus/batch-a-headless` and the tree is **confirmed green** (full `jest` + `tsc --noEmit` + `eslint`, most recently after task 13 — 636 tests). Not believed-green per-branch; actually run on the merged tree.
...
13. **The expiry alarm cannot be a JS timer** (task 13 §9.4, confirmed on-device). A `setTimeout` does **not** fire while the app is backgrounded or dozing — it waits for foreground return (observed 38–45 s late). The timer *engine* is unaffected because it is timestamp-based (state is correct on return), but spec §6.2's "the app takes focus like an alarm" needs a real platform primitive — `AlarmManager`, a scheduled notification (notifee), or a foreground service — scheduled at `blockEndAtMs`. The `EpisodeExpiryScheduler` seam is already the right shape; **task 24 supplies the platform call, and it must not be `setTimeout`.**
14. **`sessions` has no in-progress status** — only `'completed' | 'abandoned'`. A running session is **born `'abandoned'`** (task 24 creates the row) so a crash leaves the truthful value; `closeSession` overwrites on a clean end. Don't add an in-progress enum value without a migration and a reason.
...
The original 6/7/9/12 spine, the headless build stretch (10, 11, 25, 26, 27, 33, 34), **and now task 13 (timer + episode lifecycle, confirmed on-device)** are all closed — the entire backend (6/7/9/11/12/13 + data layer) is complete and confirmed on the S23 FE. **What remains on the personal-ship path is the product surface: task 23 (UI/UX design) → task 24 (functional UI).** 24 depends on 23; a minimal functional pass can begin against the settled flow (spec §6) before 23's visual system lands, but 23 is cheap now and expensive to retrofit, so design-first is the right order. Both carry `P` — they need Jason and the phone. **Task 23 is also the last place the model choice matters:** the design *thinking* is Opus (frontend-design skill) or a human designer; producing components from a settled design is Sonnet.

**In flight, headless and parallel-safe:** **35** (spec fold-in, docs-only) and **36** (recurrence engine). **Migration-number watch:** task 13 took **005 / schema 2.6.0**; if task 36 adds a migration it is **006 / 2.7.0**, and 36 re-runs the prior-suite sweep (task 13's `taskCompletion.ts` header pointer is 36's to fix — see its report §3). Sequence device (`P`) batches separately from headless work.

**Three handoffs task 13 raised, each pinned to its owner below:** the JS-timer alarm finding → task 24 (constraint #13); the crash-as-friction question → task 19; and `completion_count`/`success_rate` having no writer anywhere → task 17.
