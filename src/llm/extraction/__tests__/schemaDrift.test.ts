...
const fixtures: Array<{ name: string; value: unknown; expectValid: boolean }> = [
  ...
  { name: 'floor duration_type ("at least an hour")', value: { ...baseValid(), duration_type: 'floor' }, expectValid: true },
  { name: 'duration_type not in enum', value: { ...baseValid(), duration_type: 'guess' }, expectValid: false },
  { name: 'missing duration_type', value: (() => { const rest: Record<string, unknown> = baseValid(); delete rest.duration_type; return rest; })(), expectValid: false },
